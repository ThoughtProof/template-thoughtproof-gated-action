...
Primary contract: `GatedAction` (fail-closed HBAR hand). Optional samples still present: `HederaToken` (ERC-20) and `HtsTokenCreator` (HTS precompile at `0x167`).

## GatedAction flow (this template)

1. Off-chain agent builds a proposal → `proposalHash`
2. Off-chain gate returns ALLOW + `evidenceHash` (demo script fakes this; do not call prod Sentinel from the template by default)
3. `recorder` calls `recordAllow`
4. `executor` calls `executeTransfer` only while the receipt is live

Never describe this as Hedera-verified reasoning. Chain stores hashes + roles; verdict quality is off-chain.
